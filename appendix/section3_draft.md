...
| Validation staging (v2 → v3) | v2 had all checks in one pre-split cell (Log #21, Decision #23). In v3 I specified two-stage validation (Log #25, Decision #26) | Stage A (Cell 3): 7 pre-split integrity checks + 2 pitfall prints on `df_model`. Stage B (Cell 6): 4 post-preprocessing checks on transformed arrays. Supersedes single-stage approach |
| Pitfall prints | Agent added Balance == 0 fraction and NumOfProducts counts in Cell 3 (Log #21, Decision #24) | Confirmed from Cell 3 output: Balance == 0 fraction = 0.3617 (36.17 %); NumOfProducts counts: 1 → 5 084, 2 → 4 590, 3 → 266, 4 → 60. Numbers match EDA expectations (bimodal Balance, rare products 3–4). Copied into Section 3.1 report text. |
...
| Draft structure | Agent initially used three-part layout then iteratively restructured (Log #21, Decision #23–#25) | I requested interleaved layout so the document reads top-to-bottom |
...
| Out-of-order section numbers | Agent assigned 3.1 to Split and 3.2 to Validation, causing 3.2 to appear before 3.1 in the document (Log #28, Decision #30) | Identified and fixed: sections renumbered to match notebook execution order — Cell 3 (validation) → 3.1, Cell 4 (split) → 3.2, Cell 5 (pipeline) → 3.3. Document now reads sequentially. |
