...
| Abbreviation expansion | Agent expanded abbreviations on first use throughout Section 2: EDA → Exploratory Data Analysis, IQR → interquartile range, PR-AUC → Precision–Recall Area Under the Curve, LogReg → Logistic Regression, MLP → Multi-Layer Perceptron, L1/L2 → L1 (Lasso) / L2 (Ridge) | I reviewed each expansion for correctness and confirmed subsequent uses retain the short form only | Commit `658a95c` |
